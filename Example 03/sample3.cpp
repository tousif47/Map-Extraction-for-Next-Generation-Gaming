...
void renovation2()
{
	...
	vector<Vec4i> lines;
	HoughLinesP(newTemp, lines, 1, CV_PI/180, 180, 420, 76);
...
void renovation()
{
	...
	vector<Vec4i> lines;
	HoughLinesP(newTemp, lines, 1, CV_PI/180, 180, 140, 75);
	for(size_t i = 0; i < lines.size(); i++)
	{
		Vec4i l = lines[i];
...
int main()
{
	...
	//read an image
	cv::Mat3b img_bgr = cv::imread("s3.jpg");
	...
	vector<Vec4i> lines;
	HoughLinesP(img_bw, lines, 1, CV_PI/180, 100, 25, 10);
	for(size_t i = 0; i < lines.size(); i++)
	{
		Vec4i l = lines[i];
