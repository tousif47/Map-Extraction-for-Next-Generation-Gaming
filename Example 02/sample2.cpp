...
void renovation2()
{
	...
	vector<Vec4i> lines;
	HoughLinesP(newTemp, lines, 1, CV_PI/180, 180, 300, 50);
...
void renovation()
{
	...
	vector<Vec4i> lines;
	HoughLinesP(newTemp, lines, 1, CV_PI/180, 180, 120, 20);
	for(size_t i = 0; i < lines.size(); i++)
	{
		Vec4i l = lines[i];
...
int main()
{
	...
	//read an image
	cv::Mat3b img_bgr = cv::imread("s2.jpg");
	...
	//binary
    cv::threshold(img_gray, img_bw, 105.0, 255.0, THRESH_BINARY);
	cv::threshold(img_gray, temp, 105.0, 255.0, THRESH_BINARY);
