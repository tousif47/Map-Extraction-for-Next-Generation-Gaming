...
void renovation2()
{
	...
	vector<Vec4i> lines;
	HoughLinesP(newTemp, lines, 1, CV_PI/180, 180, 250, 30);
	for(size_t i = 0; i < lines.size(); i++)
	{
		Vec4i l = lines[i];
		line(img_bw4, Point(l[0], l[1]), Point(l[2], l[3]), Scalar(0,0,255), 13, CV_AA);
	}
...
void renovation()
{
	...
	vector<Vec4i> lines;
	HoughLinesP(newTemp, lines, 1, CV_PI/180, 100, 75, 15);
	for(size_t i = 0; i < lines.size(); i++)
	{
		Vec4i l = lines[i];
...
int main()
{
	...
	//read an image
	cv::Mat3b img_bgr = cv::imread("s17.jpg");
	...
	//binary
    cv::threshold(img_gray, img_bw, 100.0, 255.0, THRESH_BINARY);
	cv::threshold(img_gray, temp, 100.0, 255.0, THRESH_BINARY);
	...
	vector<Vec4i> lines;
	HoughLinesP(img_bw, lines, 1, CV_PI/180, 100, 15, 1);
	for(size_t i = 0; i < lines.size(); i++)
	{
		Vec4i l = lines[i];
