...
void renovation2()
{
	...
	vector<Vec4i> lines;
	HoughLinesP(newTemp, lines, 1, CV_PI/180, 180, 400, 25);
...
void renovation()
{
	...
	vector<Vec4i> lines;
	HoughLinesP(newTemp, lines, 1, CV_PI/180, 180, 100, 3);
	for(size_t i = 0; i < lines.size(); i++)
	{
		Vec4i l = lines[i];
...
int main()
{
	...
	//read an image
	cv::Mat3b img_bgr = cv::imread("s1.jpg");
	...
	cv::threshold(img_gray, temp, 75.0, 255.0, THRESH_BINARY);
	...
	vector<Vec4i> lines;
	HoughLinesP(img_bw, lines, 1, CV_PI/180, 100, 15, 2);
	for(size_t i = 0; i < lines.size(); i++)
	{
		Vec4i l = lines[i];
